            ...
            <div class="col-12" style="max-width: 1020px; margin: 0 auto;">
              <q-card class="form-card">
                <q-card-section>
                  ...
                  <!-- Loading -->
                  <q-inner-loading :showing="loading" label="Abrindo chamado..." />
...
export default {
  ...
  setup() {
    ...
    // Usuário logado (mock - você pode pegar de um store/context)
    ...
    // Últimos chamados (agora buscados da API)
    ...
    // Buscar últimos chamados do usuário
    const carregarUltimosChamados = async () => {
      try {
        const response = await api.get(`/chamados/usuario/${usuarioLogado.value.email}`)
        if (response.data.success) {
          ultimosChamados.value = response.data.chamados.map(chamado => ({
            id: chamado.id,
            titulo: chamado.titulo,
            data: new Date(chamado.data_criacao).toLocaleDateString('pt-BR'),
    ...
    // Função principal para abrir chamado
      ...
      $q.dialog({
        ...
        try {
          ...
          const dadosChamado = {
            ...
            sla_resposta: prioridadeSelecionada?.sla || '',
            sla_resolucao: prioridadeSelecionada?.sla?.split('•')[1]?.trim() || ''
          }
              ...
              actions: [
                {
                  ...
                  color: 'white',
                  handler: () => {
                    // Navegar para acompanhar chamado (ajuste conforme sua rota)
                    // router.push(`/chamados/${numeroChamado}`)
